    ...
    return (
        <div className={"container mx-auto  "}>
            {productError ? <ErrorComponent error={productError} title={"Failed to load product details"}/> : <>
                <Breadcrumbs/>
                {productLoading ? <ProductDetailsSkeleton/> :
                    <div className={"grid grid-cols-2 lg:grid-cols-11 gap-5 mt-10"}>
                        <div>
                            {product?.images.length !== 1 ? product?.images.map((image, index) => (
                                ...
                                Array(5).fill("").map((_, index) => (
                                    <div key={index}
                                         className={"relative bg-secondaryColor overflow-hidden flex flex-col" +
                                             " justify-center items-center mb-5 cursor-pointer"}>
                        ...
                        <div className={"lg:col-span-5 px-10"}>
                            ...
                            <div
                                className={"mt-4 flex justify-start items-center gap-2 lg:gap-5 small-text text-text1"}>
                                <Rating value={product?.rating}/>
                                <span
                                    className={"border-r-2 pr-2 lg:pr-5 border-text1/50 "}>({product?.reviews.length} Reviews)</span>
                        ...
                        <span className={"bg-black text-white py-1 rounded-full w-5 h-5 hover:scale-110" +
                            " cursor-pointer transition-all duration-300"}></span>
                                        <span
                                            className={"bg-red-500 text-white px-2 py-1 rounded-full w-5 h-5 hover:scale-110" +
                                                " cursor-pointer transition-all duration-300"}></span>
                                        <span
                                            className={"bg-gray-500 text-white px-2 py-1 rounded-full w-5 h-5 hover:scale-110" +
                                                " cursor-pointer transition-all duration-300"}></span>
                                    </div>
                                </div>
                                <div className={"flex justify-start items-center gap-5"}>
                                    ...
                                    <div className={"flex justify-start items-center gap-3"}>
                                        {size.map((item, index) => (
                                            <span onClick={() => setSelectedSize(item)} key={index}
                                                  className={`border
                                                      " border-black/50 px-2 py-1
                                                      " rounded-sm
                                                      " text-black
                                                      " cursor-pointer
                                                      ...
                                                      " flex
                                                      " justify-center items-center font-semibold ${selectedSize === item ? "bg-secondary2 text-white" : ""}`}>{item}</span>
                                        ))}
                                    </div>
                                </div>
                                <div className={"flex justify-start items-center gap-5"}>
                                    <IncreamentBtn quantities={quantities} setQuantities={setQuantities}/>
                                    <Button onClick={handleBuyNow} btnText={"Buy Now"}
                                            className={"bg-secondary2 text-white" +
                                                " px-12"}/>
                                    <span onClick={handleWishList} className={`text-lg cursor-pointer w-8 h-8 flex
                                        " justify-center
                                        " items-center
                                        " border-black/50 border rounded-md hover:bg-secondary2 hover:text-white transition-all duration-300 ${isFavorite ? "bg-secondary2 text-white" : ""}`}><FaRegHeart/></span>
                                </div>
                                <div className={"border border-text1 rounded-md lg:w-3/4 flex flex-col justify-center" +
                                    " items-start" +
                                    " p-6"}>
                                    <div
                                        className={"flex justify-start items-center gap-3 border-b border-text1 w-full  pb-6"}>
                                        <div><img src={delivery || "https://img.icons8.com/ios/50/000000/ship.png"}
                                                  alt=""/></div>
                                        <div>
                                            <h5 className={"font-medium"}>Free Delivery</h5><p
                                            className={"underline text-xs" +
                                                " font-medium"}>{product?.shippingInformation}</p>
                                        </div>
                                    </div>
                                    <div className={"flex justify-start items-center gap-3 w-full  pt-6"}>
                                        <div><img src={returnImg || "https://img.icons8.com/ios/50/000000/return.png"}
                                                  alt=""/></div>
                                        <div>
                                            <h5 className={"font-medium"}>Return Delivery</h5><p
                                            className={"underline text-xs" +
                ...
                <div className={"mt-10 lg:mt-16"}>
                    ...
                    <div>
                        <CustomCarousel type={"product"} data={data?.products || []} error={error} settings={settings}
                                        loading={isLoading}/>
                    </div>
                </div>
            </>}
